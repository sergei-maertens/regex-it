...
class Invoice(models.Model):
    ...
    def get_absolute_url(self):
        return reverse(
            "invoices:detail", kwargs={"invoice_number": self.invoice_number}
        )
    ...
    def generate_invoice_number(self, save=True):
        ...
        self.invoice_number = "{prefix}{number}".format(
            prefix=prefix, number=next_number
        )
        if save:
            self.save()
...
class InvoiceItem(models.Model):
    ...
    project = models.ForeignKey(
        "crm.Project", null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    content_type = models.ForeignKey(
        "contenttypes.ContentType", blank=True, null=True, on_delete=models.SET_NULL
    )
